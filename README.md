...
5. Соберите проект. В папке *out* в корне проекта появятся папки *Client* и *Server*, содержащие файлы клиентского и серверного плагинов соответственно. Важно! Для случая когда нужна реализация клиентского плагина для Linux, но при этом нет возможности установить на клиентскую машину linux .net runtime необходимо скачать архив с бинарными файлами linux .net runtime с сайта Maйкрософт и распаковать его в папку etc\linux-runtime. После этого запустите etc\build.cmd.  
