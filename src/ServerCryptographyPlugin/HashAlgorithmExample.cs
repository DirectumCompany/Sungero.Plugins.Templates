...
namespace CryptographyPlugin
{
  ...
  public sealed class HashAlgorithmExample : HashAlgorithm
  {
    ...
    /// <remarks>
    /// Метод выполняет вычисление хеша. Каждая запись в криптографический хэш-алгоритм передает данные через этот метод.
    /// Для каждого блока данных этот метод обновляет состояние хэш-объекта, чтобы в конце данных возвращалось правильное значение хеша.
    /// </remarks>
