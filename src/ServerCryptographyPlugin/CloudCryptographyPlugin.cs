...
namespace CryptographyPlugin
{
   ...
   * Закрытый ключ сертификата пользователя также хранится в облаке, поэтому нет необходимости взаимодействовать с устройством пользователя через веб-агент.
   ...
   * Поэтому проверка подписи и сертификатов всегда проверяются локально на сервере приложений.
